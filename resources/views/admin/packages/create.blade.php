...
<div class="max-w-3xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <form action="{{ route('admin.packages.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        @csrf
        <div>
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Upload an image for the package.</p>
        </div>
        <div>
        ...
        <div class="flex justify-end">
                <a href="{{ route('admin.packages') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400" style="background-color: var(--brand-orange); color: white;">Back to List</a>
            <button type="submit" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700" style="background-color: var(--brand-orange); color: white;">Save Package</button>
        </div>
    </form>
</div>
@endsection
