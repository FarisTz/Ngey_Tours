...
<div class="max-w-3xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Create Tour</h1>
        <p class="mt-1 text-sm text-gray-600">Add a new tour listing for the public tours page.</p>
    </div>
    ...
    <form action="{{ route('admin.tours.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex justify-end">
            <a href="{{ route('admin.tours') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400" style="background-color: #ec6905;"  >Back to List</a>
            <button type="submit" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700" style="background-color: #ec6905;">Save Tour</button>
        </div>
    </form>
</div>
@endsection
