...
@section('subtitle', 'Manage all tour bookings')
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-lg bg-white p-6 shadow-sm border-l-4 border-brand">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600">Total Tour Bookings</p>
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600">Pending Tour Bookings</p>
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600">Completed Tour Bookings</p>
    ...
    <div class="rounded-lg bg-white p-4 shadow-sm">
        <form method="GET" class="flex flex-wrap gap-4">
            ...
            <div class="flex items-end">
                ...
                <a href="{{ route('admin.bookings.tour') }}" class="ml-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 hover:bg-gray-50">Reset</a>
            </div>
        </form>
    </div>
    ...
    {{-- <div class="flex justify-end gap-2">
        <a href="{{ route('admin.bookings.export.excel', array_merge(request()->query(), ['booking_type' => 'tour'])) }}" class="rounded-lg bg-green-600 px-4 py-2 text-white hover:bg-green-700">
        ...
        <a href="{{ route('admin.bookings.export.pdf', array_merge(request()->query(), ['booking_type' => 'tour'])) }}" class="rounded-lg bg-red-600 px-4 py-2 text-white hover:bg-red-700">
            <svg class="inline h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">
                            ...
                            @if($booking->tour && $booking->tour->location)
                                <div class="text-xs text-gray-500">{{ $booking->tour->location }}</div>
                            @endif
                        </td>
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                            ...
                            <form method="POST" action="{{ route('admin.bookings.destroy', $booking->id) }}" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this tour booking?')">
                                    Delete
                                </button>
                            </form>
                        </td>
